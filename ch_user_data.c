...
#define BUF_SIZE 8 //buffer from input
#define 

/* below i've compiled the functions that will be used.
 * this file is not currently called anywhere
 * and these functions are from the arch/um/drivers dir
 * 
 *
 * It will be helpful to put the ioctl's on a numbered basis
 * so we can monitor which is being called in which function
 *
*/
...
void read_ioctl(int fd, unsigned long* ){
	if(ioctl(,2,2) < 0){
